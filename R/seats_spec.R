...
#' @param approximation character: the approximation mode.
#' When the ARIMA model estimated by TRAMO does not accept an admissible
#' decomposition, SEATS:
#' `"None"` - performs an approximation;
#' `"Legacy"` - replaces the model with a decomposable one;
#' `"Noisy"` - estimates a new model by adding a white noise to the
#' non-admissible model estimated by TRAMO. Default="Legacy".
#' @param trend.boundary numeric: the trend boundary (rmod). The boundary beyond
...
#' @param seas.boundary numeric: the seasonal boundary (sbound). The boundary
...
#' @param seas.boundary.unique numeric: the seasonal boundary (unique),
#' (sboundatpi).
...
#' Default=0.8.
#' @param seas.tolerance numeric: the seasonal tolerance (epsphi). The tolerance
...
#' @param ma.boundary numeric: the MA unit root boundary. When the modulus of an
#' estimated MA root falls in the range \[xl, 1\], it is set to xl. Possible
#' values \[0.9,1\]. Default=0.95.
#'
